...
int gtc_get_buf_sdc(gtc_t gtc, int priority, task_t *buf) {
  ...
  if (!got_task && tc->ldbal_cfg.stealing_enabled) {
    ...
    while (!got_task && !tc->terminated) {
      ...
      TC_START_TSCTIMER(poptail); // this counts as attempting to steal
...
void gtc_print_stats_sdc(gtc_t gtc) {
  ...
  if (!getenv("SCIOTO_DISABLE_STATS") && !getenv("SCIOTO_DISABLE_PERNODE_STATS")) {
    ...
    perget       = tc->getcalls      != 0 ? TC_READ_TSCTIMER(getbuf)    / tc->getcalls      : 0;
    peradd       = tc->tasks_spawned != 0 ? TC_READ_TSCTIMER(add)       / tc->tasks_spawned : 0;
    perinplace   = tc->tasks_spawned != 0 ? TC_READ_TSCTIMER(addinplace)/ tc->tasks_spawned : 0; // borrowed
    perfinish    = rb->nprogress     != 0 ? TC_READ_TSCTIMER(addfinish) / rb->nprogress     : 0; // borrowed, but why?
    perprogress  = rb->nprogress     != 0 ? TC_READ_TSCTIMER(progress)  / rb->nprogress     : 0;
    perreclaim   = rb->nreccalls     != 0 ? TC_READ_TSCTIMER(reclaim)   / rb->nreccalls     : 0;
    perensure    = rb->nensure       != 0 ? TC_READ_TSCTIMER(ensure)    / rb->nensure       : 0;
    perrelease   = rb->nrelease      != 0 ? TC_READ_TSCTIMER(release)   / rb->nrelease      : 0;
    perreacquire = rb->nreacquire    != 0 ? TC_READ_TSCTIMER(reacquire) / rb->nreacquire    : 0;
    perpoptail   = rb->ngets         != 0 ? TC_READ_TSCTIMER(poptail)   / rb->ngets         : 0;
    ...
    printf(" %4d - TSC: get: %"PRIu64"M (%"PRIu64" x %"PRIu64")  add: %"PRIu64"M (%"PRIu64" x %"PRIu64") inplace: %"PRIu64"M (%"PRIu64")\n",
        _c->rank,
        TC_READ_TSCTIMER_M(getbuf), perget, tc->getcalls,
        TC_READ_TSCTIMER_M(add), peradd, tc->tasks_spawned,
        TC_READ_TSCTIMER_M(addinplace), perinplace);
    printf(" %4d - TSC: addfinish: %"PRIu64"M (%"PRIu64") progress: %"PRIu64"M (%"PRIu64" x %"PRIu64") reclaim: %"PRIu64"M (%"PRIu64" x %"PRIu64")\n",
        _c->rank,
        TC_READ_TSCTIMER_M(addfinish), perfinish,
        TC_READ_TSCTIMER_M(progress), perprogress, rb->nprogress,
        TC_READ_TSCTIMER_M(reclaim), perreclaim, rb->nreccalls);
    printf(" %4d - TSC: ensure: %"PRIu64"M (%"PRIu64" x %"PRIu64") release: %"PRIu64"M (%"PRIu64" x %"PRIu64") "
           "reacquire: %"PRIu64"M (%"PRIu64" x %"PRIu64")\n",
        _c->rank,
        TC_READ_TSCTIMER_M(ensure), perensure, rb->nensure,
        TC_READ_TSCTIMER_M(release), perrelease, rb->nrelease,
        TC_READ_TSCTIMER_M(reacquire), perreacquire, rb->nreacquire);
    printf(" %4d - TSC: pushhead: %"PRIu64"M (%"PRIu64") poptail: %"PRIu64"M (%"PRIu64" x %"PRIu64")\n",
        _c->rank,
        TC_READ_TSCTIMER_M(pushhead), (uint64_t)0,
        TC_READ_TSCTIMER_M(poptail), perpoptail, rb->ngets);
  }
}
...
void gtc_print_gstats_sdc(gtc_t gtc) {
  ...
  pergetmeta   = rb->nmeta         != 0 ? TC_READ_TSCTIMER_MSEC(getmeta)   / rb->nmeta         : 0.0;
  ...
  perreacquire = rb->nreacquire    != 0 ? TC_READ_TSCTIMER_MSEC(reacquire) / rb->nreacquire    : 0.0;
  ...
  eprintf("        :   get_meta   %-32stime %35s per %s\n", 
      ...
      gtc_print_mmad(buf2, "ms", TC_READ_TSCTIMER_MSEC(getmeta), 0),
  ...
  eprintf("        : reacquire  %32s  time %35s    per %s\n",
      ...
      gtc_print_mmad(buf2, "ms", TC_READ_TSCTIMER_MSEC(reacquire), 0),
